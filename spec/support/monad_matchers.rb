...
RSpec::Matchers.define :hold do |expected|
  ...
  failure_message do |actual|
    "expected that #{actual.inspect} would be a success and #{expected.inspect}"
  end
  diffable
end
...
RSpec::Matchers.define :be_none do |_expected|
  match do |actual|
    expect(actual).to be_a Dry::Monads::Maybe::None
  end
  failure_message do |actual|
    "expected NONE instead #{actual}"
  end
end

RSpec::Matchers.define :be_some do |_expected|
  match do |actual|
    expect(actual).to be_a Dry::Monads::Maybe::Some
  end
  failure_message do |actual|
